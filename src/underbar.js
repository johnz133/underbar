(function() {
  ...
  _.uniq = function(array) {
    ...
    return _.filter(array, function(item) {
        return duplicate.hasOwnProperty(item) ? false : (duplicate[item] = true);
    });
  };
  ...
  _.some = function(collection, iterator) {
    ...
    if(collection.length == 0 || _.every(collection, function(x){ return iterator ? !iterator(x) : !x; })){
      return false;
    } 
    return true;
  };
  ...
  _.delay = function(func, wait) {
    var slicedArguments = [], i = 2;
    while(i < arguments.length){
      slicedArguments.push(arguments[i++]);
    }
    setTimeout(function(){ func.apply(this, slicedArguments)}, wait);
  };
